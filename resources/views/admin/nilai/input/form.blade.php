...
@push('script')
                    ...
                    ...Object.entries(kelompokKomponen).flatMap(([jenis, komponenList]) =>
                        komponenList.map(komponen => ({
                            ...
                            render: (data, type, row) => {
                                ...
                                if (nilaiDetail !==
                                    undefined) { // nilai detail atau nilai per komponen
                                    return `<input type="${inputType}" class="form-control form-control-sm text-center input-nilai nilai_${row.id}_${jenis}"
                    name="nilai[${row.id}][${komponen.id}]" value="${nilaiDetail.nilai}" ${disabled} onchange="hitungNilai(${row.id}, '${jenis}', ${komponen.bobot})"  oninput="hitungNilai(${row.id}, '${jenis}', ${komponen.bobot})"/>`;
    ...
    id="nilai_${row.id}_${jenis}" name="nilai[${row.id}][${jenis}]" value="${nilai.nilai_akhir}" ${disabled} />`;
